...
function AdminAnnouncementScreen({ onBack, onPublish }: { onBack: () => void; onPublish: (title: string, description: string) => void }) {
  const [title, setTitle] = useState("");
  const [description, setDescription] = useState("");
    ...
    <div className="flex flex-col h-full bg-white overflow-hidden">
      ...
      <div className="shrink-0 p-3">
        <button
          ...
          className={clsx(
            "w-full h-14 rounded-full flex items-center justify-center transition-colors",
            canPublish ? "bg-[#1441cc] active:opacity-90" : "bg-black/10 cursor-not-allowed",
          )}
        >
          <span className={clsx("font-['Noto_Sans',sans-serif] font-medium text-[16px] tracking-[0.15px]", canPublish ? "text-white" : "text-black opacity-38")}>
            Publish
          </span>
        </button>
      </div>
...
function ZoneThresholdRow({ color, label, value, onChange }: { color: string; label: string; value: string; onChange: (v: string) => void }) {
  return (
    <div className="flex items-center justify-between px-4">
      <div className="flex items-center gap-2">
        <div className="size-[15px] rounded-full shrink-0" style={{ backgroundColor: color }} />
        <span className="font-['Noto_Sans',sans-serif] font-medium text-[16px] text-black leading-6">{label}</span>
      </div>
      <div className="relative w-16 h-14 border border-[#787878] rounded-[12px] flex items-center justify-center">
        <input
          value={value}
          onChange={e => onChange(e.target.value)}
          inputMode="numeric"
          className="w-full h-full bg-transparent outline-none text-center font-['Noto_Sans',sans-serif] font-medium text-[16px] text-black"
        />
        <span className="absolute right-2 font-['Noto_Sans',sans-serif] font-medium text-[16px] text-black pointer-events-none">%</span>
      </div>
    </div>
  );
}
...
function AdminGroupSettingsScreen({ onBack, onSave }: { onBack: () => void; onSave: () => void }) {
  ...
  const [redThreshold, setRedThreshold] = useState("60");
    ...
    <div className="flex flex-col h-full bg-white overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto flex flex-col gap-6 pt-2">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-2">
            <ZoneThresholdRow color={ZONES.red.border} label="Red zone attendance" value={redThreshold} onChange={setRedThreshold} />
            <ZoneThresholdRow color={ZONES.yellow.border} label="Yellow zone attendance" value={yellowThreshold} onChange={setYellowThreshold} />
            <ZoneThresholdRow color={ZONES.green.border} label="Green zone attendance" value={greenThreshold} onChange={setGreenThreshold} />
          </div>
        </div>
      </div>
    ...
    return (
      <div className="relative h-[56px]">
        ...
        <input
          value={value}
          onChange={e => onChange(e.target.value)}
          className="absolute inset-0 w-full h-full pl-4 pr-12 bg-transparent font-['Noto_Sans',sans-serif] font-medium text-[16px] text-black outline-none rounded-[12px]"
          style={ns}
        />
      ...
      >
        ...
        <div className="flex items-center justify-between p-3">
          ...
          <button
            ...
            disabled={!memberId.trim() || !phone.trim()}
            className={clsx(
              "px-6 py-2.5 rounded-full transition-opacity",
              memberId.trim() && phone.trim() ? "bg-[#1441cc] active:opacity-90" : "bg-[#1441cc] opacity-40 cursor-not-allowed",
            )}
          >
